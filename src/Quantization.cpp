...
void Quantization::inverse(const QuantizationVolumeType type, const float *input, float *output) {
  const std::size_t i_lim = this->dim_block.getNSamples();
  float *volume = get_volume(type);

  for (std::size_t i = 0; i < i_lim; ++i)
...
float *Quantization::generate_volume(const QuantizationVolumeType type) {
  ...
  for (Point4D::value_type v = 0; v < dim_block.v; ++v) {
    qv = v * weight_v;
    for (Point4D::value_type u = 0; u < dim_block.u; ++u) {
      qu = u * weight_u;
      for (Point4D::value_type y = 0; y < dim_block.y; ++y) {
        qy = y * weight_y;
        for (Point4D::value_type x = 0; x < dim_block.x; ++x) {
          qx = x * weight_x;
          switch (type) {
            case LEE: {
