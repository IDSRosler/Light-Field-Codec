...
class SubpartitionModel {
  public:
    SubpartitionModel(const int *bitstream, const Point4D &dim_block, std::vector<bool> &treeFlags, EntropyReport *csv_report);
    ~SubpartitionModel();
    void DeleteTree();
...
private:
    ...
    void MakeTree(NodeBlock *node, std::vector<bool> &treeFlags, EntropyReport *csv_report);
