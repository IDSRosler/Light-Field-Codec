...
TEST(PartialTransformTest, descriptor_several_levels)
{
    constexpr std::size_t SIZE = 15 * 15 * 13 * 13;
    ...
    for (const auto &N : {9})
    {
        ...
        {
            for (const auto &desc : convert_fbt_to_descriptor(tree->repr(), 0))
            {
                try
                {
                    std::fill(block_map, block_map + SIZE, 0);
                    auto parsed_descriptor = parse_descriptor(desc, shape, true);
                    for (const auto &pd : parsed_descriptor)
                    {
                        int _offset = calc_offset(pd.offset, stride);
                        FOREACH_4D_IDX(i, pd.shape, stride)
                        {
                            ASSERT_EQ(block_map[i + _offset], 0)
                                << "\tdescriptor: " << desc << '\n'
                                << "\tparsed_descriptor:" << pd << '\n'
                                << "\ti: " << i << '\n'
                                << "\t_offset " << _offset << '\n'
                                << "\tblock_map[]:" << block_map[i + _offset] << '\n';
                            block_map[i + _offset] = pd.transform;
                        }
                    }
                    ASSERT_TRUE(std::all_of(block_map, block_map + SIZE, [](const auto val) {
                        return val != 0;
                    })) << "\tN: " << N << '\n'
                        << "\ttree: " << tree << '\n'
                        << "\tdescriptor: " << desc << '\n';
...
TEST(PartialTransformTest, descriptor_transform_inverse_several_levels)
{
    constexpr std::size_t SIZE = 16*16*16*16;

    Point4D shape(16,16,16,16);
    Transform tx(shape);
    ...
    for (const auto &N : {1,5,9,13})
    {
        for (const auto &tree : generate_full_binary_trees(N))
        {
            for (const auto &descriptor : convert_fbt_to_descriptor(tree->repr(), 0))
            {
                try
                {
                    std::fill(t_block, t_block + SIZE, 0);
                    std::fill(r_block, r_block + SIZE, 0);
                    auto all_descriptors = parse_descriptor(descriptor, shape,  true);
                    for (const auto &desc : all_descriptors)
                    {
                        const auto &[offset, shape, type] = desc;
                        tx.md_forward(static_cast<Transform::TransformType>(type),
                                      block,
                                      t_block,
                                      offset,
                                      shape);
                        tx.md_inverse(static_cast<Transform::TransformType>(type),
                                      t_block,
                                      r_block,
                                      offset,
                                      shape);
